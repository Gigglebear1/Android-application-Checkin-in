...
public class AddEmployee extends ActionBarActivity {
	...
	public void update() {
		...
		ParseQuery<ParseObject> employeesOnJob = ParseQuery
				.getQuery("Timecard");
		...
		query.whereDoesNotMatchKeyInQuery("userName", "userName",
				employeesOnJob);

		query.findInBackground(new FindCallback<ParseObject>() {
			public void done(List<ParseObject> employeesNotOnJob,
		...
		employeesOnJob.findInBackground(new FindCallback<ParseObject>() {
			public void done(List<ParseObject> employeesNotOnJob,
				...
				ArrayList<ParseObject> arraylist = new ArrayList<ParseObject>(
						employeesNotOnJob);
				for (ParseObject element : arraylist) {
					your_array_list.add((String) element.get("firstName") + " "
							+ (String) element.get("lastName"));
				}
	...
	public void AddEmployeeClick(View view) {
		...
		if (position > 0) {
			/*
			 * //delete from spinner adapter.remove(selected);
			 * adapter.notifyDataSetChanged();
			 * 
			 * 
			 * //add to List arrayAdapter.add(selected);
			 * arrayAdapter.notifyDataSetChanged();
			 */
			...
			addEmployee.put("timeFrom", 0);
			addEmployee.put("timeTo", 0);
			addEmployee.put("userName", copy.get(position - 1).get("userName"));
			addEmployee.put("firstName", copy.get(position - 1)
					.get("firstName"));
			...
			// popup
			Toast.makeText(getBaseContext(), selected + " Added to Job",
					Toast.LENGTH_LONG).show();
		} else {
			Toast.makeText(getBaseContext(), "Cannot Add", Toast.LENGTH_LONG)
					.show();
		}
